...
// only do one ignition flame for zombie and zombie_surv
  if ( (cent->currentState.aiChar == AICHAR_ZOMBIE || cent->currentState.aiChar == AICHAR_ZOMBIE_SURV) && !firing ) { 
	...
	while ( f ) {
		...
		if ( centFlameInfo[f->ownerCent].lastSoundUpdate != cg.time ) {
			...
			if ( centFlameStatus[f->ownerCent].streamVolume ) {
				if ( cg_entities[f->ownerCent].currentState.aiChar != AICHAR_ZOMBIE && cg_entities[f->ownerCent].currentState.aiChar != AICHAR_ZOMBIE_SURV ) {
