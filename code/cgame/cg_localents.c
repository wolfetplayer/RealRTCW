...
void CG_AddFragment( localEntity_t *le ) {
	...
	if ( le->onFireStart && ( le->onFireStart < cg.time && le->onFireEnd > cg.time ) ) {
		...
		CG_S_AddLoopingSound( le->refEntity.origin, vec3_origin, cgs.media.flameCrackSound, (int)( 5.0 * flameAlpha ) );
	}
...
//----(SA)	end
	...
	if ( hasFlame ) {
		...
		if ( VectorLength( flameDir ) == 0 ) {
			...
			CG_S_AddLoopingSound( newOrigin, vec3_origin, cgs.media.flameSound, (int)( 0.3 * 255.0 * flameAlpha ) );
		} else {
			VectorNormalize( flameDir );
			// play a flame blow sound when moving
			CG_S_AddLoopingSound( newOrigin, vec3_origin, cgs.media.flameBlowSound, (int)( 0.3 * 255.0 * flameAlpha ) );
		}
	}
...
void CG_AddClientCritter( localEntity_t *le ) {
	...
	if ( le->loopingSound ) {
		if ( cg.time > le->refEntity.fadeStartTime ) {
			CG_S_AddLoopingSound( le->refEntity.origin, vec3_origin, le->loopingSound, 255 - (int)( 255.0 * (float)( cg.time - le->refEntity.fadeStartTime ) / (float)( le->refEntity.fadeEndTime - le->refEntity.fadeStartTime ) ) );
		} else {
			CG_S_AddLoopingSound( le->refEntity.origin, vec3_origin, le->loopingSound, 255 - (int)( 255.0 * ( 1.0 - alpha ) ) );
		}
	}
