...
qboolean AICast_ScriptAction_GiveWeapon( cast_state_t *cs, char *params ) {
	...
	if ( weapon != WP_NONE ) {
		...
		if (!ent->aiCharacter)
		{
			if (g_newinventory.integer > 0 || g_gametype.integer == GT_SURVIVAL)
			{
				if (weapon != WP_AIRSTRIKE && weapon != WP_ARTY && weapon != WP_POISONGAS_MEDIC && weapon != WP_DYNAMITE_ENG && weapon != WP_DYNAMITE && weapon != WP_SMOKE_BOMB) // Skip WP_AIRSTRIKE and WP_ARTY	
				{
