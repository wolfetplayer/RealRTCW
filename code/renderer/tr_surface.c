...
void RB_SurfaceFoliage( srfFoliage_t *srf ) {
	...
	for ( o = 0; o < srf->numInstances; o++, instance++ )
	{
		...
		xyz = tess.xyz_et[ tess.numVertexes ].v;
		memcpy( xyz, srf->xyz, numVerts * sizeof( srf->xyz[ 0 ] ) );
		if ( tess.shader->needsNormal ) {
			memcpy( &tess.normal_et[ tess.numVertexes ].v, srf->normal, numVerts * sizeof( srf->xyz[ 0 ] ) );
		}
		memcpy( &tess.texCoords0_et[ tess.numVertexes ], srf->texCoords, numVerts * sizeof( srf->texCoords[ 0 ] ) );
		memcpy( &tess.texCoords1_et[ tess.numVertexes ], srf->lmTexCoords, numVerts * sizeof( srf->lmTexCoords[ 0 ] ) );
		...
		// copy color
		color = (int*) tess.vertexColors_et[ tess.numVertexes ].v;
